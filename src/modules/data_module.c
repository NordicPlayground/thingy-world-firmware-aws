...
static struct cloud_data_cfg current_cfg = {
	...
	.active_wait_timeout		= DATA_ACTIVE_ONE_HOUR_SEC,
	...
	.no_data.neighbor_cell		= (IS_ENABLED(CONFIG_DATA_SAMPLE_NEIGHBOR_CELLS_DEFAULT)
					   ? false : true),
	.loc_mode			= CLOUD_CODEC_LOC_MODE_SCELL
};
...
static void data_encode(void)
{
	...
	if (neighbor_cells.queued && (current_cfg.loc_mode == CLOUD_CODEC_LOC_MODE_MCELL ||
	...
	if (neighbor_cells.queued && (current_cfg.loc_mode == CLOUD_CODEC_LOC_MODE_SCELL ||
	    current_cfg.loc_mode == CLOUD_CODEC_LOC_MODE_ALL)) {
...
static void new_config_handle(struct cloud_data_cfg *new_config)
{
	...
	if (current_cfg.loc_mode != new_config->loc_mode) {
		...
		case CLOUD_CODEC_LOC_MODE_SCELL: {
			...
			current_cfg.active_wait_timeout = DATA_ACTIVE_ONE_HOUR_SEC;
...
static void on_all_states(struct data_msg_data *msg)
{
	...
	if (IS_EVENT(msg, app, APP_EVT_DATA_GET)) {
		 ...
		 */
		LOG_INF("data_send_work rescheduled for %d sec", msg->module.app.timeout);
