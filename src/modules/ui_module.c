...
static void btn_press_window_timer_handler(struct k_timer *timer);
static void btn_press_hold_timer_handler(struct k_timer *timer);
K_TIMER_DEFINE(btn_press_window_timer, btn_press_window_timer_handler, NULL);
K_TIMER_DEFINE(btn_press_hold_timer, btn_press_hold_timer_handler, NULL);
static atomic_val_t btn1_press_cnt;
...
static void btn_press_hold_timer_handler(struct k_timer *timer)
{
	...
	ui_module_event->type = UI_EVT_BUTTON_PRESS_LONG;
...
static void button_handler(uint32_t button_states, uint32_t has_changed)
{
	if (has_changed & DK_BTN1_MSK) {
		if (button_states & DK_BTN1_MSK) {
			/* Press */
			atomic_inc(&btn1_press_cnt);
			k_timer_stop(&btn_press_window_timer);
			k_timer_start(&btn_press_hold_timer, K_SECONDS(3),K_SECONDS(0));
		} else {
			/* Release */
